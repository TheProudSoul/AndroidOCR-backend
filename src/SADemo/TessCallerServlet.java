...
public class TessCallerServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("TessCallerServlet：");
        ...
        try {
            ...
            if (imageItems != null && imageItems.size() > 0) {
                for (FileItem item : imageItems) {
                    ...
                    //索引到最后一个反斜杠
                    int start = value.lastIndexOf(separator);
                    //截取上传文件的字符串名字，加1是 去掉反斜杠，
                    String filename = value.substring(start + 1);
                    request.setAttribute("image_file", filename);
                    //写到磁盘上
                    item.write(new File(uploadPath, filename));//第三方提供的
                    File file = new File(uploadPath + separator + filename);
                    long startTime=System.nanoTime();
                    String result = TesseractExpert.testEn(file);
                    long endTime = System.nanoTime();
                    System.out.println("后台调用接口耗时========= : "+ (endTime-startTime)+"ns");
                    System.out.println("上传成功：" + filename);
                    System.out.println("上传成功：" + filename);
                    response.getWriter().print(result);//将结果返回给客户端
                }
            }

            long totalEndTime = System.nanoTime();    //获取结束时间
            System.out.println("TessCallerServlet后台全部运行时间：" + (totalEndTime - totalStartTime) + "ns");    //输出程序运行时间
        } catch (Exception ex) {
